...
public class DiskDataStoreTest extends Assert
{
	...
	public void store()
	{
		...
		dataStore = new AsynchDiskDataStore("app1", fileStoreFolder, MAX_SIZE_PER_SESSION);
		doTestDataStore();
	...
	public void sessionFolderName()
	{
		IStoreSettings storeSettings = new StoreSettings(null);
		java.io.File fileStoreFolder = storeSettings.getFileStoreFolder();
		AsynchDiskDataStore store = new AsynchDiskDataStore("sessionFolderName", fileStoreFolder,
		...
		AsynchDiskDataStore.SessionEntry sessionEntry = new AsynchDiskDataStore.SessionEntry(store, sessionId);
		sessionEntry.unbind();
